...
@utils.timeit
def export_property_card(webmap_json, feature, map_elm_json, pdf_res=100):
    ...
    if isinstance(map_elm_json, basestring):
        map_elm_json = json.loads(map_elm_json)
    ...
    for elm_name, elm in elms.iteritems():
        if elm_name.startswith('flt_') or elm_name.startswith('cur'):
            ...
            # might have to play with these values
            if elm_name == 'disclaimer':
                disc_lines = textwrap.wrap(map_elm_json.get(elm_name, ' ').format(**feature['attributes']), 65)
                if len(disc_lines) > 7:
                    disc_lines = textwrap.wrap(map_elm_json.get(elm_name, ' ').format(**feature['attributes']), 75)
                    elm.fontSize = 5
                elm.text = '\r\n'.join(disc_lines)

            elif elm_name == 'legal':
                legal_lines = textwrap.wrap(map_elm_json.get(elm_name, ' ').format(**feature['attributes']), 50)
                if len(legal_lines) > 7:
                    disc_lines = textwrap.wrap(map_elm_json.get(elm_name, ' ').format(**feature['attributes']), 55)
                    elm.fontSize = 7
                elm.text = '\r\n'.join(legal_lines)
            else:
...
##
##    webmap_js_file = os.path.join(fold, 'webmap_test.json')
##    with open(webmap_js_file, 'r') as f:
##        webmap_js = json.load(f)
##
##    feature_file = os.path.join(fold, 'feature.json')
##    with open(feature_file, 'r') as f:
##        feature = json.load(f)
##
##    map_elm_js_file = os.path.join(fold, 'map_elms.json')
##    with open(map_elm_js_file, 'r') as f:
##        map_elm_js = json.load(f)
##    export_property_card(webmap_js, feature, map_elm_js, pdf_res=50)
